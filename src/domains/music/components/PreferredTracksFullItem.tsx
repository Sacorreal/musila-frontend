...
export const PreferredTracksFullItem: React.FC<PreferredTracksFullItemProps> = ({ track, onPlay, onOpenRequest }) => {
    ...
    return (
        <div className="group relative w-full rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 transition-all p-3 sm:p-4">
            <div className="flex items-center gap-3 sm:gap-4">
                <div className="relative flex-shrink-0 cursor-pointer" onClick={() => onPlay(track)}>
                    <Image src={track.coverUrl || "/musila.png"} alt={track.title} width={56} height={56} className="rounded-lg object-cover" />
                ...
                <div className="flex items-center gap-3 sm:gap-4 flex-shrink-0">
                    <span className="text-sm text-white/60 hidden sm:block">{formatDuration(track.duration)}</span>
                        ...
                        <AnimatePresence>
                            {menuOpen && (
                                <>
                                    ...
                                    >
                                        <button onClick={() => setMenuOpen(false)} className="w-full px-4 py-2 text-left text-sm hover:bg-white/10 transition-colors">
                                            Añadir a playlist
                                        </button>
                                        <button onClick={() => setMenuOpen(false)} className="w-full px-4 py-2 text-left text-sm hover:bg-white/10 transition-colors">
                                            Ir a artista
