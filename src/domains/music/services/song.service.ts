...
export const fetchUserSongs = async (): Promise<Song[]> => {
    ...
    return [
        { id: 1, cover: "/musila.png", title: "Melodía de un Sueño", visible: true, genre: "Pop", subgenre: "Indie Pop" },
        { id: 2, cover: "/musila.png", title: "Ecos de la Ciudad", visible: true, genre: "Rock", subgenre: "Alternativo" },
        { id: 3, cover: "/musila.png", title: "Ritmo Nocturno", visible: false, genre: "Electronic", subgenre: "House" },
    ];
};
...
export interface PreferredTracksResponse {
    emptyPreferredGenres: boolean;
    items: Song[];
}

export const fetchPreferredTracks = async (token: string): Promise<PreferredTracksResponse> => {
    try {
        const res = await fetch("/api/tracks/by-preferred-genres", {
            headers: {
        ...
        if (res.status === 404) {
            return { emptyPreferredGenres: true, items: [] };
        }
        ...
        return { emptyPreferredGenres: false, items: items || [] };
    } catch (error) {
        throw error;
    }
};
