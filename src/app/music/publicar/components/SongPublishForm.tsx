...
import { songSchema } from "../../../../domains/song/types";
  ...
  const handleSongUpload = async (file: File) => {
    try {
      ...
      showSuccess(
        ...
        "La canción se subirá cuando envíes el formulario"
      );
    } catch {
      showError(
  ...
  const onSubmit = async (data: SongFormData) => {
    ...
    try {
      const hasCoverFile =
        ...
        (typeof File !== "undefined" && data.image?.[0] instanceof File);
      let coverUrl = "/logo.webp";
      if (hasCoverFile) {
        coverUrl = await uploadCoverAndGetUrl();
      }
      ...
      if (data.lyrics && data.lyrics.trim().length > 0)
        payload.lyrics = data.lyrics.trim();

      console.log("Datos del formulario:", payload);
      showSuccess(
        "Canción publicada exitosamente",
        "La canción ha sido publicada correctamente"
      );

      setValue("name", "");
      setValue("author", "");
      setValue("genre", "" as SongFormData["genre"]);
      setValue("subgenre", "");
      setValue("lyrics", "");
      setValue("image", null);
      setValue("song", null);
      ...
      showError(
        "Error al publicar la canción",
        "Hubo un problema al publicar la canción"
      );
    } finally {
        ...
        <div className="bg-card-bg dark:bg-[#1e1834] rounded-xl shadow-lg border border-[#3c3069]/20 p-6 md:p-8">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div>
              <label
                htmlFor="name"
                className="block text-sm font-medium text-text-main dark:text-gray-100 mb-2"
              >
                Nombre de la Canción *
              </label>
              <input
